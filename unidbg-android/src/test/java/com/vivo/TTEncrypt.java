...
public class TTEncrypt {
	...
	public String ttEncrypt(String str) {
		// TTEncryptUtils.callStaticJniMethodObject(emulator, method, args);
	...
	byte[] ttEncrypt() {
		if (logging) {
			...
			hookZz.instrument(module.base + 0x00000F5C + 1, new InstrumentCallback<Arm32RegisterContext>() {
				@Override
				public void dbiCall(Emulator<?> emulator, Arm32RegisterContext ctx, HookEntryInfo info) { // 通过base+offset
																											// inline
																											// wrap内部函数，在IDA看到为sub_xxx那些
					System.out.println("R3=" + ctx.getLongArg(3) + ", R10=0x" + Long.toHexString(ctx.getR10Long()));
				}
			});

			Dobby dobby = Dobby.getInstance(emulator);
			dobby.replace(module.findSymbolByName("ss_encrypted_size"), new ReplaceCallback() { // 使用Dobby inline
																								// hook导出函数
				@Override
				public HookStatus onCall(Emulator<?> emulator, HookContext context, long originFunction) {
					System.out.println("ss_encrypted_size.onCall arg0=" + context.getIntArg(0) + ", originFunction=0x"
							+ Long.toHexString(originFunction));
		...
		ByteArray array = TTEncryptUtils.callStaticJniMethodObject(emulator, "ttEncrypt([BI)[B",
				new ByteArray(vm, data), data.length); // 执行Jni方法
